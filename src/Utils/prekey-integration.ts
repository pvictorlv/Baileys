...
export class PreKeyIntegration {
	...
	private initializeAdvancedManager(): void {
		try {
			...
			this.logger.error({error: error.message}, 'Failed to initialize Advanced PreKey Manager')
	...
	async uploadPreKeys(creds: AuthenticationCreds, count?: number): Promise<void> {
		if (this.advancedManager && this.config.enableAdvancedManager) {
			try {
				...
				}, 'Pre-keys uploaded successfully via Advanced Manager')

				return
			} catch (error) {
				this.logger.error({error: error.message}, 'Advanced PreKey Manager upload failed')
			...
			try {
				...
				if (result) {
					this.logger.info({
						...
						serverCount: result.serverCount,
						uploadTime: result.uploadTime
					}, 'Pre-keys uploaded conditionally via Advanced Manager')
				} else {
				...
				this.logger.error({error: error.message}, 'Advanced PreKey Manager conditional upload failed',)
	...
	async getAvailablePreKeysOnServer(): Promise<number> {
		if (this.advancedManager && this.config.enableAdvancedManager) {
			try {
				...
				this.logger.error({error: error.message}, 'Advanced PreKey Manager server count failed')
	...
	private async legacyUploadPreKeys(creds: AuthenticationCreds, count: number): Promise<void> {
		this.logger.info({count}, 'Using legacy pre-key upload')
		...
		this.logger.info({count}, 'Legacy pre-key upload completed')
	}
