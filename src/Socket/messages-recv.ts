...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const sendMessagesAgain = async (key: proto.IMessageKey, ids: string[], retryNode: BinaryNode) => {
		...
		for (const id of ids) {
			...
			if (messageRetryManager) {
				let cachedMsg = messageRetryManager.getRecentMessage(remoteJid, id)
				if (!cachedMsg) {
					const secondary = remoteJid.includes('@s.whatsapp.net')
						? await signalRepository.getLIDMappingStore().getLIDForPN(remoteJid)
						: await signalRepository.getLIDMappingStore().getPNForLID(remoteJid)
					if (secondary) {
						logger.debug({ jid: remoteJid, id, secondary }, 'looking for message in retry cache with secondary jid')
	...
	const handleMessage = async (node: BinaryNode) => {
		...
		const {
			...
			author,
			decrypt
		} = await decryptMessageNode(node, authState.creds.me!.id, authState.creds.me!.lid || '', signalRepository, logger)
	...
	// Periodic cleanup of retry manager caches
